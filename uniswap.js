...
async function getAmountMin(amountOfEth,percent){
    ...
    const tradeWethForDai = new Trade(routeWethForDai, new TokenAmount(WETH,  amountOfEth), TradeType.EXACT_INPUT);
...
const fetch = async () => {
    ...
    const routeWethForDai = new Route([pair], WETH);
    const tradeWethForDai = new Trade(routeWethForDai, new TokenAmount(WETH,  BigInt(3E18)), TradeType.EXACT_INPUT);
    const tradeWethForDai1 = new Trade(routeWethForDai, new TokenAmount(DAI,  BigInt(22363.2E18)), TradeType.EXACT_OUTPUT);


    const routeDaiForWeth = new Route([pair], DAI);
    const tradeDaiForWeth = new Trade(routeDaiForWeth, new TokenAmount(DAI, BigInt(22583E18)), TradeType.EXACT_INPUT);
  
    console.log(tradeWethForDai.executionPrice.toSignificant(9) + " DAI for 1 WETH")
    ...
    console.log(tradeWethForDai.priceImpact.toSignificant(9) + "    Eth for ? DAL")
    const slippageTolerance = new Percent('50', '10000')
    // console.log(slippageTolerance)
    console.log(tradeWethForDai.minimumAmountOut(slippageTolerance).raw + "    123Eth for ? DAL")
    console.log(tradeWethForDai.outputAmount.toSignificant(10) + " DAL for ? Eth")
    console.log(tradeDaiForWeth.outputAmount.toSignificant(6) + " WETH for ? DAI")
  };
